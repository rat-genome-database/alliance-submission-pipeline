...
PHENOTYPE_LOC="data/$BATCH/${MOD_NAME}_${AGR_RELEASE}_phenotype.${TAXON}.json"
PHENOTYPE_FILE="${PHENOTYPE_SPEC}=@${PHENOTYPE_LOC}"
echo "$PHENOTYPE_FILE"

DISEASE_SPEC="${AGR_RELEASE}_DAF_${SPECIES}"
DISEASE_LOC="data/$BATCH/${MOD_NAME}_${AGR_RELEASE}_disease.${TAXON}.json"
DISEASE_FILE="${DISEASE_SPEC}=@${DISEASE_LOC}"
echo "$DISEASE_FILE"

BGI_SPEC="${AGR_RELEASE}_BGI_${SPECIES}"
BGI_LOC="data/$BATCH/${MOD_NAME}_${AGR_RELEASE}_gene.${TAXON}.json"
BGI_FILE="${BGI_SPEC}=@${BGI_LOC}"
echo "$BGI_FILE"

GAF_SPEC="${AGR_RELEASE}_GAF_${SPECIES}"
GAF_LOC="data/$BATCH/${MOD_NAME}_${AGR_RELEASE}_go.${TAXON}.gaf"
gzip $GAF_LOC
GAF_FILE="${GAF_SPEC}=@${GAF_LOC}.gz"
echo "$GAF_FILE"

GFF_SPEC="${AGR_RELEASE}_GFF_${SPECIES}"
GFF_LOC="data/$BATCH/${MOD_NAME}_${AGR_RELEASE}_GFF.${TAXON}.gff3"
gzip $GFF_LOC
