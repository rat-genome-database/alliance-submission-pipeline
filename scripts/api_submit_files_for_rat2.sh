...
ALLELE_LOC="data/$BATCH/${MOD_NAME}_${AGR_RELEASE}_allele.${TAXON}.json"
gzip $ALLELE_LOC
ALLELE_FILE="${ALLELE_SPEC}=@${ALLELE_LOC}.gz"
echo "$ALLELE_FILE"

VARIANT_SPEC="${AGR_RELEASE}_VARIATION_${SPECIES}"
VARIANT_LOC="data/$BATCH/${MOD_NAME}_${AGR_RELEASE}_variant.${TAXON}.json"
gzip $VARIANT_LOC
VARIANT_FILE="${VARIANT_SPEC}=@${VARIANT_LOC}.gz"
echo "$VARIANT_FILE"

EXPR_SPEC="${AGR_RELEASE}_EXPRESSION_${SPECIES}"
EXPR_LOC="data/$BATCH/${MOD_NAME}_${AGR_RELEASE}_expression.${TAXON}.json"
gzip $EXPR_LOC
...
GENE_LOC="data/$BATCH/${MOD_NAME}_${AGR_RELEASE}_gene.${TAXON}.json"
GENE_FILE="${GENE_SPEC}=@${GENE_LOC}"
echo "$GENE_FILE"

AGM_SPEC="${AGR_RELEASE}_AGM_${SPECIES}"
AGM_LOC="data/$BATCH/${MOD_NAME}_${AGR_RELEASE}_affectedGenomicModel.${TAXON}.json"
...
HTPDATASET_LOC="data/$BATCH/${MOD_NAME}_${AGR_RELEASE}_HTPDATASET.${TAXON}.json"
HTPDATASET_FILE="${HTPDATASET_SPEC}=@${HTPDATASET_LOC}"
echo "$HTPDATASET_FILE"

HTPDATASAMPLE_SPEC="${AGR_RELEASE}_HTPDATASAMPLE_${SPECIES}"
HTPDATASAMPLE_LOC="data/$BATCH/${MOD_NAME}_${AGR_RELEASE}_HTPDATASAMPLE.${TAXON}.json"
...
REFERENCE_LOC="data/$BATCH/${MOD_NAME}_${AGR_RELEASE}_REFERENCE.${TAXON}.json"
gzip $REFERENCE_LOC
REFERENCE_FILE="${REFERENCE_SPEC}=@${REFERENCE_LOC}.gz"
echo "$REFERENCE_FILE"

REFEXCHANGE_SPEC="${AGR_RELEASE}_REF-EXCHANGE_${SPECIES}"
REFEXCHANGE_LOC="data/$BATCH/${MOD_NAME}_${AGR_RELEASE}_REF-EXCHANGE.${TAXON}.json"
